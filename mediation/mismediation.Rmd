...
dichotMediateModel <- function(aDF){
    aDF$x2.d <- ifelse(aDF$x2 < mean(aDF$x2),0,1)
    
    model.m <- lm(z2 ~ x2.d, data = aDF)
    model.y <- lm(y2 ~ x2.d + z2, data = aDF)
    
    mediateModel <- try(mediate(model.m = model.m, model.y = model.y, 
                            sims = 500, boot = F,
                            treat = 'x2.d', mediator = 'z2'))
    if (identical(class(mediateModel), "try-error")){
        rezlist <- list(coef = NA, se = NA, converged = FALSE)
    } else {
        coefVec <- c(d0 = mediateModel$d0, d1 = mediateModel$d0, 
...
if(REDOSIMS){
    mediatePkgDichot <- simsem::sim(nRep = 8*10, n = 200, generate = generatingModel, 
                                          model = dichotMediateModel,
                                          multicore = TRUE, outfun = anOutfun)
    saveRDS(mediatePkgDichot, 'mediatePkgDichot.RDS')
} else {
    mediatePkgDichot <- readRDS('mediatePkgDichot.RDS')
}
summary(mediatePkgDichot)

cat(paste0("Proportion of simulations that reject null of no mediation: ",
           mediationPower <- sum(unlist(getExtraOutput(mediatePkgDichot)))/mediatePkgDichot@nRep))
```